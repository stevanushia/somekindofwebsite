...
var ajaxUrl = currentLocation.replace('view/index.php', 'library/movies-index.php');
if (ajaxUrl == currentLocation){
    ajaxUrl = currentLocation.replace('view/', 'library/movies-index.php');
}

console.log("ajax url : " + ajaxUrl);

var adminURL = currentLocation.replace('view/index.php', 'admin/');
if (adminURL == currentLocation) {
    adminURL = currentLocation.replace('view/', 'admin/');
}
...
$.ajax({
    ...
    success: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            data[i].films.forEach(f => {
                let filmMember = {
                    id: f.id,
                    background: adminURL  + f.thumbnail,
                    display_background: adminURL + f.thumbnail,
                    title: f.title,
                    description: f.description,
                    score: f.score
                };

                movie.push(filmMember);
            });
...
const AddLists = () => {    
    ...
    lists.forEach(l => {
        ...
        let listsFilms = l.films;

        let ctrf = 0;
        listsFilms.forEach(f => {
            ...
            <p>1h 25m</p>
                <div class="button-container">
                    ...
                    <div class="star" onclick="popupDescription(this)" idfilm="${f.id}" indexlist="${ctr}" indexfilm="${ctrf}">
...
{
    indexList = prop.getAttribute("indexlist");
    indexFilm = prop.getAttribute("indexfilm");
    idFilm = prop.getAttribute("idfilm");
    let film = lists[indexList].films[indexFilm];
